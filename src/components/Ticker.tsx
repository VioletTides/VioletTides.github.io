    ...
    <div className="bg-surface-card border border-white/10 overflow-hidden relative group">
      ...
      <div className="bg-black/80 px-3 py-1.5 flex justify-between items-center border-b border-amber-primary/20 relative z-10">
        <div className="flex items-center gap-2">
          ...
          />
          <span className="text-[10px] font-bold tracking-wider text-amber-primary/80 uppercase amber-text-glow">
            {title}
          </span>
        </div>
      ...
      <div className="p-3 overflow-hidden whitespace-nowrap relative z-10 bg-[#0a0800]">
        ...
        <div className={`flex gap-6 relative z-10 ${reducedMotion ? '' : 'animate-ticker'}`}>
          {[...items, ...items].map((item, idx) => (
            <span
              key={`${item}-${idx}`}
              className="text-[11px] text-amber-primary uppercase amber-text-glow font-mono tracking-widest"
            >
